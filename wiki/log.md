...
2026-06-15 | fe-dev | develop TSK-098 (EP-014/US-052) → done | packages/app/src/domain/useTabPause.ts (new), packages/app/src/domain/useSaveData.ts (new), packages/app/src/App.tsx, packages/app/src/components/Settings/Settings.tsx | Estrazione due hook custom dai componenti monolitici App.tsx (~527 LOC) e Settings.tsx (~669 LOC). useTabPause<T extends string>(engine, activeTab, selected, playTab) incapsula `useEffect` + prevTabRef + engineRef per pausa/ripresa engine al cambio tab; PausableEngine interfaccia minimale (pattern coerente con LifecycleTarget di useAppLifecycle TSK-065). useSaveData(saveService, currentRomId) ritorna `{ list, busy, message, refresh, handleExport, handleImportFile }` (firma AC2): incapsula listSaveStates filtered (US-018 AC2), exportSaveState + triggerDownload, importSave con mapping reason→messaggio user-facing (US-019 AC3); selectedSaveStateId resta in Settings come stato UI + 1 effect di preselezione su saveStates. Complessità cognitiva stimata sotto 15 in entrambi i componenti (AppContent 22→13, Settings 19→10). Test: 42 files · 459 tests passed (baseline post-SP12-W4 invariata); npx tsc --noEmit clean. Nessun test modificato (refactor strutturale puro). AC1-AC5 + DoD completi.
