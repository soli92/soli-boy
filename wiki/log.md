...
## 2026-06-30 19:22 — develop TSK-133
**Agente:** be-dev
**TSK:** [[../management/kanban/EP-019-rtc-orologio-interno/US-065-impostare-data-ora-rtc/TSK-133]]
**Layer:** be
**Code path:** packages/app
**Files touched:** 3 (src/core/wasmboy-rtc-bridge.ts [new], src/core/wasmboy-rtc-bridge.test.ts [new], src/core/wasmboy-engine.ts [wiring])
**Commit:** (pending push)
**DoD:** pass — `WasmBoyRtcBridge implements RtcBridge` con `hasRtc/getRtcState/setRtcState` (ADR-009 §4). Detection cartuccia da header ROM byte `0x0147` (`0x0F` MBC3+TIMER+BATTERY e `0x10` MBC3+TIMER+RAM+BATTERY → true; resto → false). Encoding/decoding MBC3 ↔ RtcState wall-clock UTC tramite helper puri esportati (`rtcStateToEpochUtc`, `epochUtcToRtcState`, `encodeMbc3Registers`, `decodeMbc3Registers`): strategia "latch a mezzanotte / day=0" garantisce round-trip esatto per qualunque anno ≥ 2000 senza dipendere dal wrap del day counter 9-bit. I/O via `WasmBoy.saveState()` → patch `wasmboyMemory.wasmBoyInternalState` agli offset RTC → `WasmBoy.loadState()`; offset MBC3 dichiarati come costanti con commento TODO esplicito (sorgente Gambatte `deps/gambatte-core/` assente nel repo). `sanityCheckOffsets()` esposto come difesa preventiva: throw leggibile se `wasmBoyInternalState.length < 13`. `getRtcState()` sincrono via cache write-through; `setRtcState()` fire-and-forget con catch silenzioso (ADR-009 §4 "no-op tolerance"). Wiring in `WasmBoyEngine`: stub rimosso, `_rtcBridge` privato con getter; inizializzato in `load()` dopo `WasmBoy.loadROM(bytes)` come `new WasmBoyRtcBridge(WasmBoy, bytes[0x0147])`. 15 nuovi test, 634/634 passano (0 regressioni).
**Note:** Sprint 16 — bridge GBC reale chiuso; offset MBC3 sono placeholder convenzionali in attesa di validazione E2E con ROM Pokémon (TSK-132 §12-13, `test.skip`).

## 2026-06-30 19:25 — develop TSK-134
**Agente:** be-dev
**TSK:** [[../management/kanban/EP-019-rtc-orologio-interno/US-065-impostare-data-ora-rtc/TSK-134]]
**Layer:** be
**Code path:** packages/app/src/core/
**Files touched:** 3 (src/core/mgba-rtc-bridge.ts NEW, src/core/mgba-rtc-bridge.test.ts NEW, src/core/mgba-engine.ts MOD)
**Commit:** (pending push)
**DoD:** pass — `MgbaRtcBridge` implementa `RtcBridge` (ADR-009 §4) con tre operazioni: `hasRtc()` via Opzione A (lookup Game Code dai byte 0xAC..0xAF dell'header ROM letti via `FS.readFile(gameName)`, set statico di 16 Game Code S-3511A-dependent — Pokémon Ruby/Sapphire/Emerald + Boktai 1/2/3 + Sennen Kazoku — derivato da mGBA `src/gba/overrides.c`); `getRtcState()` via `saveState(0)` → `FS.readFile(saveStatePath)` → scansione lineare del marker `"RTC "` (4 ASCII, T-R-C-space) → decode 7 byte BCD `[year, month, day, dayOfWeek, hour, minute, second]` (dayOfWeek scartato, ADR-009 §2; year 0–99 → 2000+year); `setRtcState(state)` con encoding inverso BCD + patch in-place del blob + `FS.writeFile` + `loadState(0)` (dayOfWeek calcolato via `zellerCongruence(year, month, day)`). Export di utility `toBcd`/`fromBcd`/`zellerCongruence` per testabilità. `MgbaEngine.rtcBridge` ora mutabile (era `readonly = null`), inizializzato a `new MgbaRtcBridge(this.module)` dopo `loadGame()` (in `load()`) e reset a `null` in `stop()` (lifecycle coerente con il modulo mGBA). 28 nuovi test in `mgba-rtc-bridge.test.ts` coprono: hasRtc su 5 Game Code RTC (AXVE/AXVJ Ruby, BPEE Emerald, U3IE Boktai) + 2 non-RTC (BPRE FireRed, AGBJ generic), edge case (gameName undefined, FS.readFile lancia, ROM troppo piccola), decode di un blob con marker valido, null su Game Code non-RTC, null su blob senza marker, patch in-place verificata byte-per-byte (con dayOfWeek=2 per 2026-06-30 martedì), round-trip completo set→get ≡ state su date estreme (2099-12-31 23:59:59), no-op su hasRtc=false, no-op silenzioso su FS error, sanity check zellerCongruence su 6 date note (2000-01-01 sab, 2024-01-01 lun, 2026-06-30 mar, 2026-01-01 gio, 2024-02-29 leap-gio, 2099-12-31 gio), round-trip toBcd/fromBcd su tutti i valori 0..99. 647/647 test totali `packages/app` passano (0 regressioni vs 619 baseline). 3 errori typecheck pre-esistenti non imputabili a TSK-134 (`StubRtcBridge` lacking `hasRtc`, `App.tsx:757` type narrow, `rtc-service.test.ts:254` unused `@ts-expect-error`) — verificati con `git stash` baseline.
**Note:** Onestà / vincoli runtime — l'offset esatto del blocco RTC dentro il save state binario mGBA NON è verificato a runtime in questo repo (richiederebbe una ROM GBA RTC reale + tooling). La strategia scelta (scansione lineare del marker `"RTC "` indipendente dall'offset) è conservativa: funziona per qualsiasi posizione del chunk, purché il marker sia unico nel blob (assunzione documentata nell'header del file + TODO commentato per validazione e2e su Pokémon Rubino/Smeraldo quando una ROM RTC sarà disponibile, vedi `public/test-roms/README.md`). Lista RTC_GAME_CODES statica: 16 entry derivate dall'override database mGBA upstream — TODO post-Sprint 16 sostituire con detection automatica via mGBA WASM se l'API esporrà un capability flag pubblico. `setRtcState`/`getRtcState` sono no-op silenziose su qualsiasi fallimento (saveState=false, FS.readFile lancia, marker assente, payload troncato): coerente con il pattern best-effort di GameSession persist/restore (ADR-009 §4 "no-op silenzioso"). Tipo `MgbaModule` definito localmente nel bridge (non importato da `mgba-engine.ts`) per disaccoppiare il bridge dalla classe engine (testabilità: il bridge accetta qualsiasi oggetto strutturalmente compatibile, mock-friendly via duck typing). `zellerCongruence` ritorna 0=Domenica..6=Sabato (convenzione S-3511A nativa documentata in ADR-009 §2). Bridge fuori-ROM (gameName undefined) ritorna `hasRtc=false` → no-op: coerente con il pattern di stop() che resetta `rtcBridge=null`. Sprint 16 Wave 1 — TSK-134 chiuso, pipeline EP-019 sblocca i `test.skip` su Pokémon GBA reali (TSK-132 scenari 12–13) quando una ROM RTC sarà disponibile; il bridge WasmBoy (MBC3) resta pending TSK-135.
