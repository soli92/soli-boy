...
> **Adapter Cursor** del comando `/premortem`. Esegue una premortem strutturata (analisi del rischio via prospective hindsight) su un piano o artefatto. PATTERN §3 operazione opzionale v2.16. Accetta 3 input shape (artefatto kanban / pagina wiki / descrizione libera). Invoca la skill premortem-protocol. Opt-in totale, R.P1 (output mai auto-applicato).
> Invocazione: `/premortem <args>` (Cursor custom command).
