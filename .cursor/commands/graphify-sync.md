...
> **Adapter Cursor** del comando `/graphify-sync`. Estrae knowledge graph da un code_path target via Graphify CLI (PATTERN §16 + §20.10, v2.14 Fase 2). Output raw/ + side-channel .graphify-state/. Sub-comandi sync/show/status/refresh.
> Invocazione: `/graphify-sync <args>` (Cursor custom command).
